...
export const ServiceModal: React.FC<ServiceModalProps> = ({ service, isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/75 backdrop-blur-sm p-4 animate-in fade-in duration-200">
      <div className="relative w-full max-w-2xl bg-white rounded-2xl overflow-hidden shadow-2xl border border-slate-200 max-h-[90vh] flex flex-col font-sans">
        {/* Header with image banner */}
        <div className="relative h-36 sm:h-48 bg-slate-900 overflow-hidden shrink-0">
          <img
            src={service.image}
          ...
          <button
            ...
            className={`absolute top-3 sm:top-4 ${isAr ? 'left-3 sm:left-4' : 'right-3 sm:right-4'} bg-black/40 hover:bg-black/60 text-white p-2 rounded-full transition-colors z-10`}
          >
            <X className="w-5 h-5" />
          </button>

          <div className="absolute bottom-3 sm:bottom-4 left-4 sm:left-6 right-4 sm:right-6">
            <span className="inline-block bg-[#D0201E] text-white text-[10px] sm:text-[11px] font-bold px-2.5 py-0.5 rounded tracking-wide uppercase mb-1">
        ...
        {/* Scrollable body */}
        <div className="p-4 sm:p-6 overflow-y-auto space-y-5 sm:space-y-6 text-slate-700 text-sm">
          <div>
            <h4 className="text-xs font-bold uppercase tracking-wider text-slate-400 mb-2">
              {isAr ? 'نظرة عامة والمواصفات التشغيلية' : 'Overview & Specifications'}
            </h4>
            <p className="text-slate-600 leading-relaxed">
              {isAr ? (service.longDescAr || service.longDesc) : service.longDesc}
            </p>
          </div>
          ...
          <div>
            <h4 className="text-xs font-bold uppercase tracking-wider text-slate-400 mb-3">
              {isAr ? 'الميزات والقدرات المشمولة' : 'Standard Inclusions & Capabilities'}
            </h4>
            <div className="grid grid-cols-1 gap-2">
              {((isAr && service.featuresAr) ? service.featuresAr : service.features).map((feature, idx) => (
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                <h4 className="text-xs font-bold uppercase tracking-wider text-slate-400">
                  {isAr ? 'نماذج من بنود الكتالوج والمخزون' : 'Sample Catalog Items'}
                </h4>
                <span className="text-xs text-slate-400 flex items-center gap-1">
                  <FileSpreadsheet className="w-3.5 h-3.5 text-sky-500" />
                  <span>{isAr ? 'أكواد IMPA / ISSA وقوائم مخصصة' : 'IMPA / ISSA / Custom List'}</span>
                </span>
              </div>
              <div className="border border-slate-200 rounded-lg overflow-x-auto text-xs">
                <table className="w-full text-start">
                  <thead className="bg-slate-100 text-slate-600 uppercase text-[10px] font-bold">
                    <tr>
                      <th className="px-3 py-2 text-start">{isAr ? 'كود الصنف' : 'Item Code'}</th>
                      <th className="px-3 py-2 text-start">{isAr ? 'الوصف والتفاصيل' : 'Description'}</th>
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {service.sampleItems.map((item, idx) => (
                      <tr key={idx} className="hover:bg-slate-50">
                        <td className="px-3 py-2 font-mono text-sky-700 font-semibold" dir="ltr">
                          <span className="inline-block unicode-isolate font-mono">{item.code}</span>
                        </td>
          ...
          <div>
            <h4 className="text-xs font-bold uppercase tracking-wider text-slate-400 mb-2">
              {isAr ? 'معايير الجودة والامتثال البحري' : 'Quality & Regulatory Compliance'}
            </h4>
        ...
        <div className="p-4 bg-slate-50 border-t border-slate-200 flex items-center justify-between gap-3 shrink-0">
          ...
          >
            <span>{isAr ? 'طلب تسعير لهذه الخدمة' : `Request Quote for ${service.title.split('&')[0]}`}</span>
