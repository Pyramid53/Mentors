        ...
        {submittedQuoteId ? (
          <div className="max-w-2xl mx-auto bg-white rounded-2xl shadow-xl border border-emerald-200 p-8 text-center animate-in zoom-in-95 duration-200">
            ...
            <div className="mt-6 bg-slate-50 border border-slate-200 rounded-xl p-4 flex items-center justify-around text-xs">
              <div>
                <span className="text-slate-400 block uppercase font-bold text-[10px]">Guaranteed Response</span>
              ...
              <div>
                <span className="text-slate-400 block uppercase font-bold text-[10px]">Duty Officer Assigned</span>
                <span className="text-[#0B2545] font-bold text-sm mt-0.5 block">Capt. Tarek (Suez Desk)</span>
              </div>
            </div>
            ...
            <div className="mt-6 flex flex-wrap items-center justify-center gap-3">
              ...
              <button
                ...
                className="bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold text-sm px-5 py-2.5 rounded-lg transition-colors"
              >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
            {/* LEFT COLUMN: Tabbed Form (7 Cols on desktop) */}
            <div className="lg:col-span-8 bg-white rounded-2xl shadow-md border border-slate-200 overflow-hidden">
              ...
              <form onSubmit={handleSubmit} className="p-6 sm:p-8 space-y-6">
                ...
                {activeTab === 'basic' && (
                  <div className="space-y-5 animate-in fade-in duration-200">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div>
                        <label className="block text-xs font-bold uppercase tracking-wider text-slate-700 mb-1.5">
                          Port of Call / Anchorage <span className="text-red-500">*</span>
                        </label>
                        <select
                        ...
                        >
                          <option value="Suez Anchorage (V-Zone)">Suez Anchorage (V-Zone / South)</option>
                          <option value="Port of Suez / Port Tawfik">Port of Suez & Port Tawfik</option>
                          <option value="Ain Sokhna Port">Ain Sokhna Port</option>
                          <option value="Adabiya Commercial Port">Adabiya Commercial Port</option>
                          <option value="Port Said (Northbound Convoy)">Port Said (Northbound Convoy)</option>
                          <option value="Port Said West / East SCCT">Port Said East (SCCT) / West</option>
                          <option value="Damietta Port">Damietta Port</option>
                          <option value="Alexandria / Dekheila">Alexandria / Dekheila</option>
                        </select>
                      </div>
                    ...
                    {/* Services Required Checkboxes (Blueprint spec) */}
                    <div>
                      <label className="block text-xs font-bold uppercase tracking-wider text-slate-700 mb-2">
                        Services Required (Check all that apply) <span className="text-red-500">*</span>
                      </label>
                      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                        {[
                          'Fresh & Frozen Provisions',
                          'Dry Stores & Beverages',
                          'Technical Stores (IMPA)',
                          'Bonded Stores & Tobacco',
                          'Crew Welfare & SIMs',
                          'Safety & Marine Chemicals',
                        ].map((srv) => {
                    ...
                    <div className="pt-3 flex justify-end">
                      <button
                        type="button"
                        onClick={() => setActiveTab('list')}
                        className="bg-[#0B2545] hover:bg-[#12345C] text-white text-xs sm:text-sm font-bold px-6 py-2.5 rounded-lg transition-colors"
                      >
                        Continue to Provision List →
                      </button>
                    </div>
                  </div>
                )}
                  ...
                  <div className="space-y-6 animate-in fade-in duration-200">
                    <div>
                      <h3 className="text-sm font-bold text-[#0B2545] mb-1">
                        Upload Your Requisition Document
                      </h3>
                      ...
                      >
                        ...
                        <div className="flex flex-col items-center justify-center pointer-events-none">
                          ...
                          <p className="text-sm font-bold text-slate-800">
                            Drag & drop your requisition file here
                          </p>
                      ...
                      {isUploading && (
                        <div className="mt-3 bg-white p-3 rounded-lg border border-slate-200 shadow-sm">
                          <div className="flex items-center justify-between text-xs mb-1">
                            <span className="font-semibold text-slate-700">Uploading requisition file...</span>
                            <span className="font-mono text-sky-600 font-bold">{uploadProgress}%</span>
                          </div>
                      ...
                      {formData.fileName && !isUploading && (
                        <div className="mt-3 bg-emerald-50 border border-emerald-200 p-3 rounded-lg flex items-center justify-between text-xs">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-emerald-700 font-mono">({formData.fileSize})</span>
                          </div>
                          <button
                            type="button"
                    ...
                    <div>
                      <div className="flex items-center justify-between mb-2">
                        <label className="text-xs font-bold uppercase tracking-wider text-slate-700">
                          Or Select Common Standard Supplies
                        </label>
                ...
                {activeTab === 'additional' && (
                  <div className="space-y-5 animate-in fade-in duration-200">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      ...
                      <div>
                        ...
                        <input
                          type="email"
                          required
                          placeholder="e.g. master.anjifortune@shipping.com"
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div>
                        <label className="block text-xs font-bold uppercase tracking-wider text-slate-700 mb-1.5">
                          Direct WhatsApp / Mobile Phone <span className="text-red-500">*</span>
                        </label>
                        <input
                          type="tel"
                          required
                      ...
                      <div>
                        <label className="block text-xs font-bold uppercase tracking-wider text-slate-700 mb-1.5">
                          Shipping Company / Ship Manager
                        </label>
                        <input
                          type="text"
                          placeholder="e.g. Apex Marine Management Ltd."
                    ...
                    <div>
                      ...
                      <div className="grid grid-cols-3 gap-2">
                        {(['Standard (60 Min)', 'Urgent (< 30 Min)', 'Anchorage Delivery'] as const).map((p) => (
                          <button
                            type="button"
                            key={p}
                    ...
                    {/* Additional Notes Textarea (Blueprint spec) */}
                    <div>
                      <label className="block text-xs font-bold uppercase tracking-wider text-slate-700 mb-1.5">
                      ...
                      <textarea
                        rows={3}
                        placeholder="Any special requests: Halal certification requirements, customs bonded delivery notes, reefer launch preference, crew diet notes..."
                ...
                {/* Primary Submit Button (Blueprint: Red Submit Request button) */}
                <div className="pt-4 border-t border-slate-200">
                  ...
                  >
                    {isSubmitting ? (
                      <>
                        ...
                        <span>Sending to 24/7 Operations Desk...</span>
                      </>
                    ) : (
                      <>
                  ...
                  <p className="text-[11px] text-center text-slate-500 mt-2 flex items-center justify-center gap-1.5">
                    ...
                    <span>No obligation. Transparent IMPA/ISSA pricing delivered within 60 minutes.</span>
                  </p>
                </div>
              </form>
            </div>

            {/* RIGHT COLUMN: Process Stepper & Immediate Assistance Card (Blueprint Page 2 right side) */}
            <div className="lg:col-span-4 space-y-6">
              {/* Vertical Process Stepper (Blueprint: 1 Submit Request, 2 We Review, 3 Get Quotation, 4 Confirmation) */}
              ...
              {/* "Need Immediate Assistance?" Mini Card (Blueprint Page 2) */}
              <div className="bg-gradient-to-br from-[#0B2545] to-[#12345C] text-white rounded-2xl p-6 shadow-md">
                ...
                <div className="bg-white/10 rounded-xl p-3 mb-4 space-y-1">
                  <span className="text-[11px] text-slate-300 block">Suez Operations Hotline:</span>
                  <a
                ...
                {/* Green WhatsApp-Style Button (Blueprint spec) */}
                <a
